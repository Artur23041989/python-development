...
class Parser:
    ...
    def get_html(self):
        ...
        try:
            ...
            if response.status_code == 200 or str(response.status_code).startswith('3'):
    ...
    def parse_html(self):
        if self.get_html() is not None:
            soup = Bs(self.html, "html.parser")
            day = soup.find('div', class_="dates short-d").text
            self.data_weather[day] = {}
    ...
    def write_data_to_file(self):
        with open("parser_oop.json", "w") as f:
            json.dump(self.data_weather, f, indent=2, ensure_ascii=False)
